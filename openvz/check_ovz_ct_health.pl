...
sub get_perfdata {
	...
	#if ($name eq "physpages" || $name eq "swappages") {
	#	$val .= sprintf(" %s=%s%s", $label.'_committed', get_limit($name), get_uom($name));		
	#	$label .= '_used';
	#}
	
	$val .= sprintf(" %s=%d%s;%s;%s;%s;%s", $label, convert($name, $RESOURCE->{$name}->{'held'}),
		get_uom($name), get_barrier($name), get_limit($name), "", get_limit($name));
...
if (! -f "/etc/vz/conf/$OPTS{c}.conf") {
	# Try match by hostname...
	my $file;
	chomp($file = `/bin/fgrep -l \"HOSTNAME=\\\"$OPTS{c}\" /etc/vz/conf/*.conf | head -n1`);
	if (defined($file) && -f "$file" && $file =~ m/^.*\/([0-9]+)\.conf$/) {
		$OPTS{c} = $1;
	} 
	else 
	{
...
print "Healthy on " . hostname(). " ".$output." |".$perfdata, "\n";
exit $ERRORS{$state};
